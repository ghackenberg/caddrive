...
// ATTACHMENT
// TODO: übergabeparameter auf attachmentid anpassen

export async function canFindAttachmentOrFail(user: User, commentId: string, issueId: string) {
...
export async function canCreateAttachmentOrFail(user: User, commentId: string) {
    const comment = await getCommentOrFail({ id: commentId, deleted: null }, NotFoundException)
    const issue = await getIssueOrFail({ id: comment.issueId, deleted: null }, NotFoundException)
    try {
        await getProductOrFail({ id: issue.productId, public: true, deleted: null }, Error)
...
export async function canReadAttachmentOrFail(user: User, attachmentId: string) {
    const attachment = await getAttachmentOrFail({ id: attachmentId, deleted: null }, NotFoundException)
    const comment = await getCommentOrFail({ id: attachment.commentId, deleted: null }, NotFoundException)
    const issue = await getIssueOrFail({ id: comment.issueId, deleted: null }, NotFoundException)
    try {
        await getProductOrFail({ id: issue.productId, public: true, deleted: null }, Error)
    } catch (error) {
...
export async function canUpdateAttachmentOrFail(user: User, attachmentId: string) {
    const attachment = await getAttachmentOrFail({ id: attachmentId, deleted: null }, NotFoundException)
    const comment = await getCommentOrFail({ id: attachment.commentId, deleted: null }, NotFoundException)
    const issue = await getIssueOrFail({ id: comment.issueId, deleted: null }, NotFoundException)
    try {
...
export async function canDeleteAttachmentOrFail(user: User, attachmentId: string) {
    const attachment = await getAttachmentOrFail({ id: attachmentId, deleted: null }, NotFoundException)
    const comment = await getCommentOrFail({ id: attachment.commentId, deleted: null }, NotFoundException)
    const issue = await getIssueOrFail({ id: comment.issueId, deleted: null }, NotFoundException)
    try {
