...
from caddrive.ldraw import LEN_LDU, NUM_LDU_X, NUM_LDU_Y, NUM_LDU_Z
...
class Part():
    ...
    lenxSeg = NUM_LDU_X * LEN_LDU        #20 * lenLDU    # Element length in x-direction
    lenySeg = NUM_LDU_Y * LEN_LDU        #20 * lenLDU    # in y-direction
    lenzSeg = NUM_LDU_Z * LEN_LDU        #8 * lenLDU     # in z-direction  (3 segments for "normal brick")
    ...
    def __init__(self, partName, partId, nSegx, nSegy, nSegz, posx, posy, posz):
        ...
        # Nodes
        self.nodesId = numpy.zeros((self.num_elx+1, self.num_ely+1, self.num_elz+1))   # id
        self.nodesx = numpy.zeros((self.num_elx+1, self.num_ely+1, self.num_elz+1))  # coorx
        self.nodesy = numpy.zeros((self.num_elx+1, self.num_ely+1, self.num_elz+1))  # coory
        self.nodesz = numpy.zeros((self.num_elx+1, self.num_ely+1, self.num_elz+1))  # coorz

        # Elements
        numberElements = self.num_elx * self.num_ely * self.num_elz
        self.elements = numpy.zeros((numberElements, 8))

        # Group of Nodes: Array of integer with the node numbers
        self.ntop   = numpy.zeros( (self.num_elx+1)*(self.num_ely+1) )    # All nodes on top
        self.nbot   = numpy.zeros( (self.num_elx+1)*(self.num_ely+1) )    # All nodes on botttom
        self.nleft  = numpy.zeros( (self.num_ely+1)*(self.num_elz+1) )    # All nodes left
        self.nright = numpy.zeros( (self.num_ely+1)*(self.num_elz+1) )    # All nodes right
        self.nfront = numpy.zeros( (self.num_elx+1)*(self.num_elz+1) )    # All nodes front
        self.nback  = numpy.zeros( (self.num_elx+1)*(self.num_elz+1) )    # All nodes back

        self.nubtop  = numpy.zeros((self.nSegx, self.nSegy))    # Nub node ids on top
        self.nubtopx = numpy.zeros((self.nSegx, self.nSegy))    # Nub on top: coorx
        self.nubtopy = numpy.zeros((self.nSegx, self.nSegy))    #                 y
        self.nubtopz = numpy.zeros((self.nSegx, self.nSegy))    #                 z

        self.nubbot  = numpy.zeros((self.nSegx, self.nSegy))    # Nub node ids on botttom
        self.nubbotx = numpy.zeros((self.nSegx, self.nSegy))    # Nub on botttom: coorx
        self.nubboty = numpy.zeros((self.nSegx, self.nSegy))    #                     y
        self.nubbotz = numpy.zeros((self.nSegx, self.nSegy))    #                     z

        # Faces (for contact)
        self.ftop   = numpy.zeros((self.num_elx*self.num_ely, 4))    # Faces on top (QUAD 4)
        self.fbot   = numpy.zeros((self.num_elx*self.num_ely, 4))    # Faces on bottom (QUAD4)
        self.fleft  = numpy.zeros((self.num_ely*self.num_elz, 4))
        self.fright = numpy.zeros((self.num_ely*self.num_elz, 4))
        self.ffront = numpy.zeros((self.num_elx*self.num_elz, 4))
        self.fback  = numpy.zeros((self.num_elx*self.num_elz, 4))
