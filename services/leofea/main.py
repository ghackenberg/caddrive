...
def index():
    ...
    request.files["ldr"].save(f"{WORKDIR}/{JOBNAME}.ldr")
    request.files["lib"].save(f"{WORKDIR}/{JOBNAME}.lib")

    # Read LDR file and get table of lego parts as return
    lM = leoFeaModelDescription.leoFeaModelDescription()
    tableLeoFeaModel = lM.readFileLDR(f"{WORKDIR}/{JOBNAME}.lib", f"{JOBNAME}.ldr")
    ...
    reqDataA = MultipartEncoder(
        fields = {
            "mail": (f'{JOBNAME}.mail', open(f'{WORKDIR}/{JOBNAME}.mail', 'rb'), 'text/plain'),
            "comm": (f'{JOBNAME}.comm', open(f'{WORKDIR}/{JOBNAME}.comm', 'rb'), 'text/plain')
        }
    )
    ...
    with open(f"{WORKDIR}/{JOBNAME}.resu", "wb") as file:
        file.write(resDataA.parts[0].content)
    with open(f"{WORKDIR}/{JOBNAME}.message", "wb") as file:
        file.write(resDataA.parts[1].content)
    with open(f"{WORKDIR}/{JOBNAME}.rmed", "wb") as file:
        file.write(resDataA.parts[2].content)

    # Send rmed file to ParaView and receive rendered image
    reqDataB = MultipartEncoder(
        fields = {
            "rmed": (f'{JOBNAME}.rmed', open(f'{WORKDIR}/{JOBNAME}.rmed', 'rb'), 'text/plain')
        }
    )
    ...
    with open(f"{WORKDIR}/{JOBNAME}.png", "wb") as file:
        file.write(resB.content)

    # Return rendered image
    return send_file(f"{WORKDIR}/{JOBNAME}.png")
...
def postprocess():
    ...
    request.files["resu"].save(f"{WORKDIR}/{JOBNAME}.resu")
    request.files["message"].save(f"{WORKDIR}/{JOBNAME}.message")
    ...
    m = MultipartEncoder(
        fields = {
            'resultsMinMax': (f'{JOBNAME}.resultsMinMax', open(f'{WORKDIR}/{JOBNAME}.resMinMax.feather', 'rb'), 'text/plain')
