...
class leoFeaPostProcess():
    ...
    def postProcessStatic(self):
        ...
        # Store results
        self.pd_DeplNoda.to_feather(f"{OUTPUTS_DIR}/{self.jobName}.depl.feather")
        self.pd_ForcNoda.to_feather(f"{OUTPUTS_DIR}/{self.jobName}.forc.feather")
        self.pd_resultsMinMax.to_feather(f"{OUTPUTS_DIR}/{self.jobName}.resMinMax.feather")
        self.pd_resultsMinMax.to_csv(f"{OUTPUTS_DIR}/{self.jobName}.resMinMax")
    ...
    def _convertResu2Pandas(self):
        ...
        fin = open(f'{OUTPUTS_DIR}/{self.jobName}.resu', 'r')
    ...
    def _readMessageSimulationTime(self):       # Read simulation time from codeaster message file
        ...
        messagefile = f'{OUTPUTS_DIR}/{self.jobName}.message'
